'use client';
/* Native document navigation preserves imported anchors. The asset pipeline pre-optimizes images to WebP. */
/* oxlint-disable next/no-html-link-for-pages, next/no-img-element */
import { useEffect, useMemo, useRef, useState } from 'react';
import { Dialog, DialogContent, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Sheet, SheetContent, SheetTitle, SheetDescription, SheetTrigger } from '@/components/ui/sheet';
import { CinematicExtras } from '@/components/cinematic-extras';
...
/* Stable identity: React 19 re-assigns innerHTML whenever the dangerouslySetInnerHTML object changes, which would wipe the motion classes and DOM enhancements on every re-render. */
const footerHtml={__html:footer.html};
...
export default function Website({page}:{page:Page}) {
  ...
  const pageHtml=useMemo(()=>({__html:page.html}),[page.html]);
  const open=(value:Modal)=>{opener.current=document.activeElement as HTMLElement;setStatus('');setModal(value);};
  const close=()=>{setPlaying(false);setModal(null);setStatus('');timers.current.push(setTimeout(()=>opener.current?.focus(),250));};
  const galleryStep=(delta:number)=>setModal(m=>m?.images?{...m,index:((m.index||0)+delta+m.images.length)%m.images.length}:m);
  useEffect(()=>{if(!playing)return;const interval=setInterval(()=>{if(!document.hidden)galleryStep(1);},4500);return()=>clearInterval(interval);},[playing]);
  useEffect(()=>{const pending=timers.current;const scroll=()=>setScrolled(window.scrollY>35);window.addEventListener('scroll',scroll,{passive:true});return()=>{window.removeEventListener('scroll',scroll);pending.forEach(clearTimeout);};},[]);
  useEffect(()=>{
    ...
    const getCard=(el:Element)=>{let node:Element|null=el.parentElement;while(node&&node!==root){if(node.querySelector('h2,h3'))return node;node=node.parentElement;}return null;};
    ...
    const submit=(form:HTMLFormElement)=>{
      ...
      let result=form.querySelector<HTMLElement>('.form-status');if(!result){result=document.createElement('output');result.className='form-status';result.setAttribute('aria-live','polite');form.appendChild(result);}
    ...
    root.addEventListener('click',e=>{
      ...
      if(button.dataset.choice){e.preventDefault();button.parentElement?.querySelectorAll('[data-choice]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));const form=button.closest('form');if(form){let field=form.querySelector<HTMLInputElement>('input[name=element]');if(!field){field=document.createElement('input');field.type='hidden';field.name='element';form.appendChild(field);}field.value=button.dataset.choice;}return;}
      if(button.closest('form')&&button.getAttribute('type')==='submit')return;
      if(button.dataset.motionControl!==undefined)return;
      if(button.dataset.photoTour!==undefined||/play_arrow|pause|volume_|fullscreen|closed_caption|Watch .*Tour|Watch Suite/.test(text)){e.preventDefault();showGallery();return;}
  ...
  return <>
    ...
    <div className="mobile-booking"><span>Your Himalayan escape</span><button className="primary-button" onClick={()=>open({kind:'inquiry',title:'Book Your Stay'})}>Book Your Stay</button></div>
    <div ref={content} className="stitch-content" data-page={page.slug}><div dangerouslySetInnerHTML={pageHtml}/><CinematicExtras page={page.slug}/><div dangerouslySetInnerHTML={footerHtml}/></div>
